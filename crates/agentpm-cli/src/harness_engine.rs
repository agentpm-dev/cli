...
fn embedding_request_trace_fields(
    action: &SemanticAction,
    runtime: &RuntimeSnapshot,
    action_source: Option<&str>,
...
fn embedding_failure_code(result: &ActionDispatchResult) -> Option<String> {
    let code = result
        .output
        .get("error")
...
mod tests {
    ...
    use crate::harness_runtime::model::{
        KnowledgeEmbeddingSnapshot, ModelProviderSelection, ModelRuntimeFailure, ModelTurn,
        RuntimeCapabilitySnapshot, ScriptedModelRuntime, SkillResourceSnapshot,
