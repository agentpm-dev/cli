...
The AgentPM local index is an implementation-owned search structure generated by `agentpm knowledge build` for vector-mode Knowledge packages. Its public manifest type is `agentpm-local`. The index enables `agentpm knowledge query` to find the nearest chunk vectors for a query vector and return ranked chunk/source results.

The canonical portable data remains chunks, sources, embedding metadata, and vector rows. The index may be rebuilt from those canonical files. Phase 6B should treat the index as a generated acceleration artifact, not the source of truth.
...
In Phase 6B, the `agentpm-local` index and `agentpm knowledge query` command are intentionally lightweight developer-experience utilities. They make vector-mode Knowledge artifacts immediately inspectable and queryable after install without requiring consumers to install or configure an external vector database. The canonical package data remains the declared chunks, sources, embedding metadata, and vector file. The local index/query path proves the artifact is usable and provides a simple default retrieval path, but it is not intended to replace future runtime-specific retrieval integrations or backend vector database exporters.
