...
21. Run the same representative Agent through headless, machine/SDK, and TUI modes and compare Engine outcomes/phase transitions. Record the comparison with `scripts/harness_release_verify.py` using at least these cases: `headless`, `node-sdk`, `python-sdk`, and `tui`; use `HARNESS_RELEASE_VERIFICATION.md` as the manual runbook.
...
- cross-surface Harness equivalence evidence generated by:
  ```bash
  python3 scripts/harness_release_verify.py \
    --case headless=headless:<headless-report.json>:<headless-events.jsonl> \
    --case node-sdk=node-sdk:<node-report.json>:<node-events.jsonl> \
    --case python-sdk=python-sdk:<python-report.json>:<python-events.jsonl> \
    --case tui=tui:<tui-report.json>:<tui-events.jsonl> \
    --evidence <evidence-dir>/harness-surface-equivalence.json
  ```
  Follow `HARNESS_RELEASE_VERIFICATION.md` for the artifact-generation steps. The evidence JSON must be retained with the release notes or test artifacts. Do not pass `--allow-empty` for release evidence; every case should include non-empty phase summaries and trace event counts.
