...
- [ ] Implement `key`, `filter`, `chronological`, and practical local `full_text` retrieval only where declared by the Blueprint and advertised by the selected runtime.
- [ ] Implement TTL anchor `(updated_at ?? created_at) + ttl`, update-refresh behavior, lazy expiry on startup/read/write/retrieval where practical, and exclusion of expired records from active reads/counts/retrieval.
