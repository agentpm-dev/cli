...
## Milestone 20A: Phase Output Preservation for Explicit Phase Completion
> Scope note: make user-facing phase and final Run output reliable across headless, machine/SDK, and TUI surfaces before final documentation/example hardening. This milestone owns the HarnessEngine/observability semantics that preserve a phase's latest assistant answer as `PhaseResult.output` when an explicit `phase_completion` action omits `output`, including both intermediate phase transitions and terminal completion. It should not add new TUI presentation features; TUI should benefit by reading the same canonical phase/result/report output as every other surface.
- [ ] Add an Engine-level fallback for explicit phase completion with no `output`: if the completing phase has latest assistant content in the phase-local transcript, use that assistant content as the `PhaseResult.output`. When that phase terminates the Run, the same fallback output should flow through terminal output.
