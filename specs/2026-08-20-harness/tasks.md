...
### Output preservation
- [ ] Add an Engine-level fallback for explicit phase completion with no `output`, using an explicit precedence ladder rather than "latest assistant content". In order: (1) explicit `phase_completion.output` when present; (2) otherwise non-empty `assistant_content` from the same `ModelTurn` that proposed the accepted `PhaseCompletion`; (3) otherwise the latest non-empty assistant content in the current phase execution, but only when no accepted semantic action or action result has occurred after it; (4) otherwise no fallback output. When that phase terminates the Run, the selected output flows through terminal output unchanged.
- [ ] Do not select "the last assistant entry" unconditionally. A phase that reads `Assistant("I'll query the deployment status") -> ToolCall -> ToolResult -> PhaseCompletion(output: null)` would promote pre-action commentary — an intention — as if it were the phase's conclusion, which is worse than `null` because it reads as an answer. Step 3's "no accepted action since" guard exists precisely to reject that case. Make this exact scenario a test.
- [ ] Implement step 2 without new transcript metadata: `ModelTurn` carries `assistant_content: Option<String>` and `actions: Vec<SemanticActionProposal>` in the same struct (`harness_runtime/model.rs:1652`), so the Engine already holds the completing turn's assistant content when it accepts the `PhaseCompletion`.
- [ ] Implement step 3's exclusions by typed variant, not by inspecting content. `TranscriptEntryKind` already distinguishes `UserInput`, `Assistant`, `ActionResult`, and `RepairFeedback` (`harness_runtime/model.rs:29`), so action results and repair feedback are excluded by construction. Do not sniff content strings for markers: this codebase has twice regressed into substring-based classification (Milestone 14c error kinds, the Milestone 18 MCP failure categorizer), and fallback selection is exactly the kind of rule that invites it. The non-empty requirement matters because a model turn consisting only of action calls must not promote an empty string into phase or terminal output.
- [ ] Preserve explicit `phase_completion.output` as authoritative when present; the fallback must only apply when completion output is absent/null.
- [ ] Keep implicit-complete behavior unchanged: no-action implicit completion may continue using assistant content as output through the existing path.
- [ ] Ensure the fallback is scoped to the current phase execution only, so assistant content from prior phases, prior Runs, repair turns, or action results cannot become phase or terminal output accidentally.
- [ ] Emit/report the resulting output through the existing canonical paths (`PhaseResultReady`, terminal event, `RuntimeTerminalResult`, `RunReport.terminal_output`, and trace/report artifacts) rather than adding TUI-only recovery logic.
- [ ] Add tests for explicit completion with prior assistant content and missing output, explicit completion with provided output, explicit completion with no assistant content, multi-turn phases where the latest assistant answer should win, and multi-phase Runs where a previous phase's assistant content must not leak into the terminal phase.
