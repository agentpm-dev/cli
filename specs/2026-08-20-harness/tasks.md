...
## Milestone 14f: Direct Memory Hooks, Observability, Redaction, SDK Parity, and Final M14 Acceptance
...
- [ ] Make `before_memory_read` live after an authorized direct read action is selected and before runtime execution; safe input includes the authorized Blueprint/package/space/retrieval request and trusted scope snapshot.
- [ ] Allow `before_memory_read` Hooks to shape query/filter/limit/mode only within the already-bound/ready space and declared retrieval modes; Hooks cannot change authoritative scope, package, space, record type, or grant undeclared modes.
- [ ] Make `before_memory_write` live after an authorized direct write action is selected and before runtime execution; safe input includes authorized Blueprint/package/space/record type plus proposed content and trusted scope snapshot.
- [ ] Allow `before_memory_write` Hooks to patch record `content` or reject; Hooks cannot set or patch envelope/scope/ID/timestamps/provenance/schema/package/space authority.
...
- [ ] Emit Memory readiness/direct read/write/validation/retention/capacity/vector/suppression events according to trace policy, with stable event names usable by TUI, machine clients, RunReport summaries, and SDK consumers.
- [ ] Ensure Memory content, queries, filters, model-visible result text, scope values, provenance details, and Hook inputs/results obey the existing trace content redaction policy; secret-bearing provider/env data must never appear in generic events.
- [ ] Include Memory request counts, backend/runtime failures, suppression reasons, and direct read/write summaries in RunUsage, SessionUsage, events, machine responses, and RunReport summaries.
- [ ] Verify Node/Python SDK host-provider and Hook registrations can carry the direct Memory request/response/hook contracts without parsing raw JSONL or losing typed errors.
- [ ] Add final M14 integration tests covering restart persistence, arbitrary trusted scopes, generated-contract enforcement, invalid-write repair, all three space models, append-only, TTL/archive/delete, capacity, retrieval-mode readiness, persistence governance across local SQLite, process/host custom runtimes, Hook-modified Memory content, semantic-vector generation, and restart/readback, semantic vector lifecycle/provider availability, explicit custom-runtime no-fallback, Hook patch/rejection/revalidation, redaction, SDK/machine parity, and `.agentpm` immutability.
...
- [ ] Add optional `memory.write_review.points` to `agentpm.harness.json` version 1 with only `phase_end | run_end`; require a non-empty unique array when `write_review` is present, reject duplicate/unknown values, and treat omitted `write_review` as persistence review disabled.
- [ ] Keep `memory.write_review` as Harness execution behavior only; it must not be written into or interpreted as part of portable Agent, Loop, Profile, Skill, Knowledge, or Memory manifests.
- [ ] Treat `phase_end` as a review after a valid `PhaseCompletion` has been produced and validated but before Harness accepts that completion and leaves the current phase.
- [ ] Treat `run_end` as a review before accepting a valid `PhaseCompletion` whose resolved Loop transition is an authored terminal (`$end`, `$handoff`, or `$abort`).
- [ ] If both `phase_end` and `run_end` are configured at a terminal phase boundary, execute only `run_end`; do not perform duplicate reviews for the same pending completion.
- [ ] Do not invoke persistence review for runtime termination caused by failure, cancellation, `limit_reached`, unresolved `approval_required`, or another termination that is not driven by an accepted authored `PhaseCompletion`.
- [ ] Skip a configured review point without calling `ModelRuntime` when the current `EffectivePhase` exposes no ready/authorized direct Memory write surface; emit an explicit skip reason.
- [ ] Preserve the original valid `PhaseCompletion` as pending while persistence review runs. Persistence review must not alter the phase outcome, `PhaseCompletion` output, resolved transition, phase identity, Loop graph/checkpoints, or other substantive `PhaseResult` semantics.
- [ ] Treat persistence review as an isolated review context tied to exactly one pending `PhaseCompletion`; the review sub-loop may perform multiple authorized `MemoryRead`/`MemoryWrite` actions within its bounds, but Harness must not re-enter review recursively or start a second review sub-loop for the same pending completion, including after review Memory writes, repair attempts, or review failure handling.
- [ ] After successful review completion, accept the original pending `PhaseCompletion` and continue the normal `PhaseResult`/transition/terminal path.
- [ ] Keep the persistence-review working transcript separate from ordinary phase result semantics after control returns; committed Memory writes and observability remain durable/visible, but review model chatter and review-local action results must not change `PhaseResult` output or become hidden context for later phases except through explicit committed Memory and normal report/trace surfaces.
- [ ] Keep persistence review entirely outside Loop-step accounting: it does not enter another phase and does not consume another Loop step.
- [ ] Implement persistence review as a bounded `ModelRuntime` sub-loop using the existing provider-neutral `ModelRequest`/`ModelTurn` machinery rather than a separate Memory-review protocol.
- [ ] Assemble persistence-review requests through the existing canonical prompt architecture, retaining the current phase's relevant Harness control context, authored phase/Profile/loaded-Skill behavior, Run input plus Consumer Context, prior `PhaseResult`s, and current phase-local transcript.
...
- [ ] Preserve the current phase-local transcript for review so model/Tool/MCP/Knowledge/Memory results already learned during the phase can inform persistence decisions; do not load unrelated prior Run transcripts or hidden provider conversation state.
- [ ] Derive the review capability catalog from the current phase's `EffectivePhase` and expose only currently authorized/ready `MemoryRead` descriptors, currently authorized/ready `MemoryWrite` descriptors, and Harness-owned `PersistenceReviewComplete`.
- [ ] Do not expose AgentPM Tools, imported MCP Tools, Knowledge requests, new Skill resource-read actions, ordinary `PhaseCompletion`, or other semantic actions during persistence review.
- [ ] Keep already-loaded Skill instructions/resources in authored behavioral context, but do not allow persistence review to progressively load additional Skill resources.
- [ ] Never expand Memory authority for the review: a space readable but not writable remains read-only, a writable-but-unreadable space remains write-only, unbound/suppressed spaces remain unavailable, and Harness config must not override Loop direct Memory access restrictions.
- [ ] Implement `PersistenceReviewComplete` as a Harness-owned semantic control action normalized through `ModelRuntime` like other semantic actions but executed directly by Harness rather than by `ToolRuntime`, `MemoryRuntime`, or another Runtime Service.
- [ ] Validate every proposed persistence-review action against the narrowed review catalog before accepting, counting, or executing it; rejected out-of-catalog proposals must not mutate Memory or advance the pending phase completion.
- [ ] Reject/repair provider responses that attempt to use actions outside the narrowed persistence-review catalog rather than silently treating the review as a normal phase turn.
- [ ] Route persistence-review `MemoryRead` through the exact normal direct Memory pipeline: authority/readiness validation -> `before_memory_read` Hook -> revalidation -> `MemoryRuntime` -> structured result.
- [ ] Route persistence-review `MemoryWrite` through the exact normal direct Memory pipeline: authority/readiness validation -> canonical content validation -> `before_memory_write` Hook -> revalidation -> persistence-governance durable projection -> durable-projection validation -> `MemoryRuntime` -> structured result.
- [ ] Preserve ordinary direct mutation semantics during review, including authorized existing-record selectors for update/delete/archive where applicable; do not introduce a review-specific write operation or backend API.
- [ ] Append Memory read/write results produced during review to the persistence-review working transcript so later review calls can use them, for example to read an existing note before choosing update versus create.
- [ ] Keep review-time Memory actions subject to all normal scope authority, generated-contract validation, retention/capacity rules, custom-runtime routing/no-fallback behavior, Hook behavior, and backend failure semantics established by Milestones 14a-14f.
- [ ] Add bounded persistence-review safety accounting separate from ordinary phase continuation so the optional review cannot become an unbounded post-phase agentic loop.
- [ ] Count persistence-review `ModelRuntime` calls in normal `RunUsage`/`SessionUsage` model-call, token, cost, and duration accounting.
- [ ] Count accepted persistence-review `MemoryRead`/`MemoryWrite` actions in normal Memory/action usage accounting; they are Memory actions, not Tools and not Loop steps.
- [ ] Include bounded malformed-response/action repair for the review using existing structured repair machinery where practical.
- [ ] On persistence-review-specific limit exhaustion or unrecoverable review `ModelRuntime`/service failure, emit a review failure/warning and proceed with the already-valid pending `PhaseCompletion` rather than converting an otherwise completed phase into a Harness runtime failure solely because optional persistence review failed.
- [ ] Preserve any Memory writes already successfully committed before a later review failure according to normal `MemoryRuntime` transaction semantics; do not invent cross-action rollback across already-completed independent Memory writes.
- [ ] Emit stable persistence-review lifecycle events, including at least `memory_write_review_started`, `memory_write_review_completed`, `memory_write_review_skipped`, and `memory_write_review_failed`.
- [ ] Include review point, phase execution ID, `ModelRuntime` call count, Memory reads/writes attempted/completed, and completion/skip/failure reason in review event/report metadata while keeping content governed by existing trace/redaction policy.
- [ ] Let the TUI, machine protocol, Node/Python SDK event streams, JSONL trace, and `RunReport` observe persistence-review activity through the existing event/report surfaces; do not create a separate review-specific client transport.
- [ ] Include persistence-review summaries in `RunReport`/usage data sufficiently to distinguish Harness-initiated review Memory activity from Memory actions the model chose naturally during ordinary phase execution.
- [ ] Add tests for omitted `write_review` preserving existing behavior, `phase_end` review before transition, `run_end` review before `$end`, `$handoff`, and `$abort`, `run_end` superseding `phase_end` at terminal boundaries, one review per pending completion with no recursive re-entry, review skipped with no writable Memory, readable+writable review performing read -> write -> complete, writable-only review without unauthorized reads, narrowed catalog rejecting Tool/Knowledge/Skill-resource/normal-`PhaseCompletion` actions before accept/count/execute, original `PhaseCompletion`/outcome/transition remaining unchanged, review working transcript not changing `PhaseResult` or later-phase hidden context, normal Memory Hooks and durable-projection governance applying to review writes, custom MemoryRuntime routing/no fallback during review, review model/action accounting without additional Loop steps, review limit exhaustion, review `ModelRuntime`/Memory failure behavior, committed writes surviving a later nonfatal review failure, persistence-review lifecycle events/report summaries, and behavior parity across built-in SQLite and fake process/host MemoryRuntime fixtures.
