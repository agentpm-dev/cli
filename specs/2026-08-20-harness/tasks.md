...
- [ ] Add Ratatui frontend as the default TTY mode for `agentpm harness` and start it early enough to render an initial shell plus current readiness state instead of leaving the terminal blank until readiness completes. Detailed incremental bootstrap/preflight/service progress belongs to Milestone 19B.
- [ ] Add explicit terminal-capability surface selection: when neither `--machine` nor `--headless` is supplied, start the TUI only if stdout/stderr are usable interactive terminals and the terminal is not an unsupported mode such as `TERM=dumb`; otherwise fall back to headless or fail before terminal setup with an actionable message that names `--headless`/`--machine`.
...
- [ ] Implement lightweight branding from config: visible title, optional subtitle, optional `#RRGGBB` accent with validation and safe terminal fallback; invalid/unsupported color values must fall back to the default accent and emit a warning. Branding never alters protocol/event/report/package identity.
