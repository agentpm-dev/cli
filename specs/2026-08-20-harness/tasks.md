...
- [ ] Surface per-Run Consumer Context metadata from the canonical run events/report in run-specific Trace/Reports views, including loaded/unavailable state, path, byte size, approximate tokens, and hash where available. Keep the left readiness rail as a preflight/bootstrap snapshot rather than a live file watcher, so users can distinguish "workspace was configured" from "this Run used this context snapshot."
- [ ] Make reports/output retrieval usable despite alternate-screen mode: provide a clear path from the compact Run output box to the expanded output viewer and from there to the durable report/trace file paths, so users can recover or copy assistant output without relying on terminal scrollback. Add a native copy action only if platform support is reliable; otherwise prefer explicit file/path recovery over a flaky clipboard abstraction.
