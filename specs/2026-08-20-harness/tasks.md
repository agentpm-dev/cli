...
- [ ] Honor explicit `memory.packages` mappings where package keys are versionless AgentPM identities resolved to exact versions from `agent.lock`; unmapped packages use the built-in SQLite runtime.
- [ ] Initialize configured process and host MemoryRuntime implementations through the common runtime/service descriptors and protocols from `spec.md`; process services must handshake before readiness, and host services without a connected registration are unavailable.
- [ ] Require custom MemoryRuntime initialization to return the correct role, compatible protocol version, `ready:true`, a normalized live capability descriptor, and package/version/Blueprint realization readiness for explicitly mapped packages.
- [ ] Reuse the Milestone 12 activation/readiness pattern for MemoryRuntime: startup/host registration/capability/Blueprint realization results must feed per-package/space readiness before direct Memory surfaces are exposed, not be rediscovered as late dispatch failures after model selection.
- [ ] Apply readiness/capability enforcement to process and host custom runtimes: reject `ready:false`, registry/package/version/Blueprint realization mismatches, and malformed capability payloads before exposure.
...
- [ ] Compare custom runtime capabilities to every bound direct space and suppress only the affected spaces where safe, retaining explicit reasons in preflight, EffectivePhase, trace, report, and machine output.
- [ ] Apply persistence governance and durable-projection validation before dispatch to built-in, process, or host `MemoryRuntime` implementations; custom MemoryRuntime implementations must never receive `x-agentpm-persist:false` values as part of a durable create/update/upsert mutation request.
- [ ] Guarantee the same persistence-governance boundary regardless of whether the selected backend is SQLite, a process service, an SDK-hosted provider, or a later PostgreSQL/Redis implementation.
- [ ] Route direct read/write operations for mapped packages only to the configured custom runtime and never silently fall back to SQLite on startup, attestation, capability, timeout, or backend failure.
- [ ] Isolate runtime failures per configured runtime/package so one unhealthy custom runtime does not disable unrelated healthy custom runtimes or local unmapped packages.
- [ ] Emit equivalent service health diagnostics/events for process and host MemoryRuntime startup, handshake, request, timeout, failure, cancellation, and unavailable/suppressed states.
- [ ] Add tests for process and host custom readiness, host-not-registered suppression, `ready:false`, package/version/Blueprint mismatch, malformed capabilities, direct-space descriptor/primitive suppression, lifecycle-only capability gaps not suppressing direct access, fake process/host custom runtimes receiving only governed durable projections, one-bad-runtime isolation, explicit custom-runtime no-fallback, mapped operation routing, backend failure result mapping, and service health events.
