...
- [ ] Add a TUI Session controller that owns the existing `HarnessSession`/`HarnessEngine`/runtime-service lifecycle and presents UI-safe state snapshots to Ratatui.
- [ ] Reuse the same bootstrap/preflight pipeline as headless and machine mode, but emit incremental bootstrap/preflight/service readiness updates early enough for the TUI shell to render progress.
- [ ] Add or extend UI-facing state models for workspace readiness, current Run status, phase status, usage, approval state, reports, traces, service surfaces, and selected visible panel without exposing mutable Engine internals.
- [ ] Add a UI-facing workspace-readiness summary model rather than relying on ad hoc renderer grouping: stable category nouns/counts for Tools, Knowledge, Memory spaces/operations, Hooks, MCP imports/exports, pending live checks, suppressed/unavailable reasons, byte sizes, approximate token counts, and resolved-value source tags.
- [ ] Add a single event subscription/fan-out path for the TUI that receives the same canonical Harness events as traces and machine clients, with trace content policy and secret redaction already applied before rendering.
- [ ] Add run-control methods for start Run, cancel Run, approval approve/deny, and external Memory-operation control that delegate to the existing Engine/control ingress rather than mutating `RunState` directly.
- [ ] Add a terminal-safe reporting adapter that can locate/read the current Run's report and trace paths for the center `Trace` and `Reports` tabs without bypassing report/trace content policy.
- [ ] Add state-transition handling for active -> terminal that preserves latest user-facing output, terminal status, report path, trace path, and usage metadata before the composer reopens.
- [ ] Keep all TUI-facing state read-only except explicit control requests; Ratatui must not own Loop traversal, action execution, Hook behavior, approval semantics, Memory lifecycle semantics, or report writing.
- [ ] Add Rust tests for TUI controller/state-reducer behavior using fake engine/events or existing deterministic runtime fixtures rather than terminal rendering. State reducer behavior and active -> terminal transition handling are required test coverage, not optional rendering-adjacent checks.
...
- [ ] Add left-rail overflow behavior for smaller terminals: scroll, collapse, or prioritize readiness groups so diagnostics and critical unavailable/suppressed state remain reachable when the rail is taller than the viewport.
