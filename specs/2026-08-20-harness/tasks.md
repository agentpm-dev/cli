...
- [ ] Keep Run-start service activation, including MCP import connect/initialize and other `run-services` setup that happens after plan resolution, off the TUI render/input loop. While the next Run is starting, render progress/status updates for service activation and failures instead of freezing the shell; unreachable or slow MCP imports must remain cancellable/quit-safe and visible as startup progress/errors rather than reintroducing the bootstrap-era blank/frozen terminal problem.
...
Covered milestones: 20.
This gives us realistic Templates/examples, full documentation, and end-to-end release verification across headless, machine/SDK, and TUI surfaces after the Ratatui UI has already been proven in Release Band 8.
