...
- [ ] Define and persist enough operation-state detail to make lifecycle execution deterministic and restart-safe: armed/disarmed status, trigger baseline, next eligibility time where applicable, last successful completion, last failure, and source/output bookkeeping needed for provenance and cascade decisions.
- [ ] Implement and test every schema-defined `memoryTrigger` variant from `agentpm.manifest.schema.json`: `external` (`type`), `record_count` (`type`, `space`, `threshold`), `capacity` (`type`, `space`), and `interval` (`type`, `every`), including rejection of invalid trigger property/type combinations.
- [ ] Treat `record_count.space` and `capacity.space` as referenced operation spaces for readiness, trusted scope resolution, and eligibility even when the monitored space is not otherwise an input/output/target of the operation.
...
- [ ] Implement and test every schema-defined `transform` operation property from `agentpm.manifest.schema.json`: `type`, `description`, `trigger`, single-entry `inputs`, `output`, `source_handling`, optional/defaulted `output_mode` (`create | replace_input`), and `preserve_provenance`, including rejection of invalid property/type combinations.
- [ ] Implement and test every schema-defined `consolidate` operation property from `agentpm.manifest.schema.json`: `type`, `description`, `trigger`, one-or-more unique `inputs`, `output`, `source_handling`, and `preserve_provenance`, including rejection of invalid property/type combinations.
- [ ] Implement and test every schema-defined `delete` operation property from `agentpm.manifest.schema.json`: `type`, `description`, `trigger`, one-or-more unique `targets`, and `cascade_derived_records`, including rejection of invalid property/type combinations.
...
- [ ] Implement `cascade_derived_records` for delete deterministically for both `true` and `false`: when true, delete downstream records derived from the target records using Harness-owned provenance/source links; when false, delete only the declared target records.
...
- [ ] Pin deterministic source-record selection and ordering for transform/consolidate lifecycle model prompts so repeated runs, repairs, and restarts produce stable requests.
- [ ] Keep lifecycle model request/report shape explicit: canonical lifecycle context is available in diagnostics, provider-facing requests include only the actionable lifecycle generation context needed by the runtime, and accepted model output is target `content` only.
...
- [ ] Pin the failure propagation matrix for automatic, external, and capacity-relief-triggered operations, including which failures are nonfatal diagnostics, which fail the external control call, and which must fail the originating Memory write/phase because capacity or atomicity could not be preserved.

- [ ] Make `before_memory_operation` Hook live after an operation is eligible and before execution; Rust HookRuntime, machine protocol, and Node/Python SDK typed Input/Decision shapes must match `spec.md`: safe input includes only the eligible participating operation, safe source summary, and trusted scope; allowed decision is reject/allow plus operation-model guidance where applicable.
- [ ] Invoke `before_memory_operation` before transform/consolidate model generation and lifecycle staging so operation-model guidance can shape the lifecycle model request; for delete, invoke it before mechanical delete mutation.
- [ ] Revalidate `before_memory_operation` Hook decisions before lifecycle execution and reject any patch/result that attempts to rewrite trigger, inputs/output/targets, source handling, operation type, or scope.
...
- [ ] Add tests for durable trigger persistence/re-arm, every trigger variant/property set, 12th-record mid-phase consolidation, capacity relief/failure, interval baseline across process restarts, transform create/replace, deterministic transform/consolidate source ordering, consolidate/delete, delete cascade true/false, source handling/provenance, `before_memory_operation` Hook allow/reject/guidance plus forbidden patch rejection across Rust/machine/SDK shapes, invalid lifecycle output repair, operation-state restart recovery, operation failure mapping, external invocation through canonical control, Loop direct-memory restrictions not blocking internal operations, and transactional rollback consistency.
- [ ] Add staged-execution tests covering source records changed, archived, or deleted between staging and commit, embedding failure before commit, commit failure rolling back output/source/vector/trigger-state together, trigger state unchanged when a staged operation never commits, capacity relief staged before the direct write transaction opens rather than inside it, a crash between staging and commit leaving no durable trace, and a concurrent writer blocking only for the short commit window rather than for the model-call phase; use fake slow model/embedding runtimes plus deterministic lock/timeout probes rather than wall-clock assumptions.
