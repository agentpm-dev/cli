...
- [ ] Capture the current baseline first: record representative repeated-action traces showing that accepted semantic actions and their results reach the provider only as rendered transcript prose, so the improvement can be measured rather than assumed.
- [ ] Replace the single-string provider prompt contract with a provider-neutral ordered turn structure capable of representing system/control context, assistant content, accepted semantic action calls, and their corresponding action results, while keeping one canonical `ModelRequest` as the source of truth.
- [ ] Build provider-native action/result correlation in the built-in OpenAI, Anthropic, and Ollama-compatible adapters, including provider call/tool identities where the API supports them (`function_call` -> `function_call_output`, `tool_use` -> `tool_result`, or the provider-native equivalent).
- [ ] Ensure the canonical `ModelRequest` exposes the same ordered assistant/action/result/correlation information to process and host custom ModelRuntime implementations, so custom providers can preserve equivalent native correlation against whatever backing service they use without depending on built-in `ProviderRequest` types. Harness owns serialization for built-in providers; for custom providers Harness owns the canonical request and the consumer owns provider serialization.
- [ ] Carry provider-native call identities from a `ModelTurn` back into the next request so each action result is attributed to the call that produced it; never fabricate, reuse, or reorder call identities.
- [ ] Degrade deliberately for providers or models without native structured action/result turns: use the strongest available representation and record the chosen strategy, rather than silently falling back to undifferentiated prose.
- [ ] Treat the canonical Harness phase transcript as authoritative execution history. Provider serialization must not omit, reorder, weaken, or ambiguously associate successful action results when reconstructing subsequent requests.
- [ ] Give every logical `ModelRequest` component exactly one provider-wire representation. Each transport needs an explicit serialization plan in which Section 5 capability descriptors become structured action declarations, Run input becomes the leading native user turn, Section 6 history becomes ordered native assistant/action/result turns, and Harness control, authored behavior, Consumer Context, and prior PhaseResults become system/instruction/context material. When Run input is emitted as the leading native user turn, omit that same Run-input content from the provider text rendering of Section 3 while retaining Consumer Context and other contextual material there; do not duplicate any logical component between base prompt text and ordered native turns.
- [ ] Map every `TranscriptEntryKind` onto a native turn when native correlation is in use, so switching representation drops no history: run/user input as the leading user turn, assistant content as an assistant turn, accepted semantic actions as native action calls, action results as correlated native action results, and repair feedback as a model-visible turn.
- [ ] Give `RepairFeedback` exactly one model-visible provider-wire representation per request. When native ordered turns carry repair feedback, do not additionally append the same feedback to Harness-control prose; when native turns are unavailable, preserve the existing deliberate control-text fallback. Repair semantics and repair-budget accounting remain unchanged. `RepairFeedback` is currently both a transcript entry and appended to Harness control text, so mapping it to a turn without removing the prose injection would reproduce the double-history problem this milestone exists to eliminate.
- [ ] Omit the Section 6 phase-local transcript prose from provider request text whenever the selected transport carries that same history as native action/result turns, exactly as Milestone 14c.1 omits the Section 5 Effective Capability Catalog when equivalent structured action declarations are sent. Sending both is worse than sending either: the native turns say "you called this and received this," while the prose block says "here is a list of things that happened," and a model reading both can treat the prose as additional history and double-count completed work.
- [ ] Derive the transcript-omission decision structurally from the transport's chosen representation strategy, not by inspecting rendered prompt text or matching section titles. Follow the Milestone 14c.1 correction where `ProviderRequest.include_capability_catalog` carries the decision explicitly.
- [ ] Keep the diagnostic/logical render complete. `render_text()`-equivalent full rendering must continue to include Sections 5 and 6 for traces, reports, debugging, and tests even when the provider request carries that content as structured declarations and native turns.
- [ ] Preserve the M14c.1 invariant: adding native action/result turns must not reintroduce a duplicated Effective Capability Catalog or detailed action schemas into ordinary prompt prose.
- [ ] Treat the provider request-shape change as an intentional trace-visible change; `ModelRuntimeRequestPrepared` snapshots and verbose trace/debug rendering must show the turn structure actually sent, and canonical action/result identities must remain present in machine/SDK/report diagnostics.
- [ ] Update Node/Python SDK host-model-provider documentation to show the recommended serialization of the canonical `ModelRequest` into provider-native action/result turns. Host and process model providers receive the canonical request rather than a built-in `ProviderRequest`, so their wire shape does not break, but the docs currently show only a canned stub response and give no serialization guidance. Without it, the obvious implementation is to flatten `prompt.sections` into a single user message, which reproduces exactly the missing-correlation behavior this milestone removes from the built-in transports.
- [ ] Add tests covering the turn structure sent to each built-in transport, provider-native action/result ID correlation across consecutive turns, action/result ordering and one-to-one association, every `TranscriptEntryKind` surviving the switch to native turns including `RepairFeedback`, Section 6 transcript prose omitted from provider text when native turns carry the same history, Run input appearing only as the leading native user turn and not also in Section 3 provider text, repair feedback appearing exactly once per request across native turns and control prose, no double representation of any logical `ModelRequest` component, full logical/diagnostic rendering still including Sections 5 and 6, deliberate degradation when native correlation is unavailable including transcript prose correctly retained in that case, unchanged canonical dispatch/authority/normalization, and no native-provider prose-catalog regression.
- [ ] Add one representative end-to-end fake-provider multi-turn phase test covering action -> native action result -> distinct next action -> native action result -> PhaseCompletion, verifying that logical transcript, provider wire request, events, usage accounting, and RunReport remain aligned.
