...
- [ ] Record assistant-content presence on completing turns in the measurement pass, because this change has one plausible backfire: some models emit less prose when compelled to call a tool, which would trade a missing action for a missing answer and undercut precedence step 2. Sequence this change *before* the measurement pass rather than after — once a tool is required the repair path should rarely fire, so "how often does the missing-completion repair produce output" stops being the informative number, and the questions worth measuring become whether phase output survives and whether the extra model request is gone.
