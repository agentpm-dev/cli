...
- [ ] Keep provider-native action declarations authoritative for action aliases, canonical identities, descriptions, schemas, enum constraints, and argument validation. Prompt prose may describe how/when to use actions, but must not become a duplicate schema source of truth.
- [ ] Update built-in OpenAI, Anthropic, and Ollama-compatible provider serialization so requests with non-empty native structured actions do not also send the full Effective Capability Catalog prose block by default.
...
- [ ] Add `trace.include_logical_capability_catalog` to `agentpm.harness.json` trace configuration, defaulting to `true`, so diagnostic trace/report/debug prompt rendering continues to include Section 5 unless explicitly disabled.
- [ ] Ensure trace content policy and unconditional secret redaction still apply to logical prompt renderings, provider text renderings, and structured action declarations.
- [ ] Add tests proving native provider requests send structured tools/functions without duplicating the detailed catalog in prompt text, while trace/report/debug logical rendering includes Section 5 by default and honors `include_logical_capability_catalog:false`.
