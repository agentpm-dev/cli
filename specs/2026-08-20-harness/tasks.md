...
- [ ] Add tests for ready/suppressed descriptor computation, global/phase binding and selector union behavior, Loop direct read/write restrictions, trusted arbitrary scope resolution and unresolved-scope suppression, provider-facing schema compatibility plus authoritative validation after schema simplification, invalid-write repair before mutation, filter path validation and unknown-path repair, fake-path removal, phase-local transcript insertion, action-limit accounting, and backend failure mapping.
