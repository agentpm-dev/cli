...
- [ ] Parse and validate `memory.local.semantic`; `embedding_provider` must reference `providers.embeddings`, `model` is the requested embedding model, and `dimensions` is a positive integer in the local vector-space identity.
- [ ] Advertise local `semantic` retrieval only when the configured EmbeddingProvider is ready and can satisfy the exact provider/model/dimensions/normalization space required by the local runtime; otherwise omit `semantic` with an explicit readiness reason even if an embedding provider exists elsewhere.
- [ ] Store little-endian contiguous IEEE-754 `f32` vectors and canonical durable-projection content hashes in `memory_vectors`, isolated by package/version/space/scope/record/provider/model/dimensions.
- [ ] Generate, update, invalidate, and delete vectors transactionally with record mutation where part of the same semantic write.
- [ ] Generate persisted Memory content hashes, semantic embedding input, semantic vectors, full-text/index material, and any other durable content-derived representation only from the durable content projection.
- [ ] Ensure values governed by `x-agentpm-persist:false` never influence persisted embeddings, vectors, hashes, durable search/index artifacts, or semantic retrieval results; this persistence rule is separate from trace redaction policy.
- [ ] Treat a content-hash mismatch as a stale vector; regenerate through the configured EmbeddingProvider before returning that record from semantic retrieval, or fail/suppress with a typed readiness/result reason when regeneration is impossible.
- [ ] Treat an active authorized record with no vector for the selected embedding space as requiring lazy vector generation before semantic ranking; missing vectors and stale vectors follow the same bounded embedding/error path and must not make otherwise-valid records permanently invisible merely because they predate semantic-runtime readiness.
- [ ] For a semantic read, embed the authorized query through the configured EmbeddingProvider, restrict candidates to active records in the authorized package/space/record-type/scope/filter domain, materialize/regenerate required vectors, then perform deterministic cosine ranking.
- [ ] Perform exact cosine similarity in Rust for Phase 7B local Memory semantics and return deterministic ranked results with stable record identity/provenance.
- [ ] Account for EmbeddingProvider requests in usage/events and preserve the configured timeout/cancellation/error mapping used by the existing embedding provider path.
- [ ] Add tests for semantic readiness omitted without config/provider, exact embedding-space compatibility, vector blob dimensions/endianness, transactional vector write/update/delete/invalidation, durable-projection-only embedding input/content hashes/index material, changing only an ephemeral/non-persistable value not leaking into persisted embedding input/vector-derived state or durable indexes, missing-vector backfill, stale hash regeneration, query embedding/candidate restriction flow, provider failure mapping, deterministic cosine ranking, no sqlite-vec dependency, and embedding usage/event accounting.
