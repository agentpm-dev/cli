...
- [ ] Split diagnostic logical prompt rendering from provider text rendering: the full logical render may include Section 5 for trace/report/debug/tests, while native provider request text may omit the detailed Effective Capability Catalog when equivalent structured action declarations are sent.
- [ ] Keep Harness's canonical `ModelRequest` authoritative for action aliases, canonical identities, descriptions, schemas, enum constraints, and argument validation. Provider-native action declarations are the authoritative wire representation of that request; prompt prose may describe how/when to use actions, but must not become a duplicate schema source of truth.
- [ ] Update built-in OpenAI and Anthropic provider serialization so requests with non-empty native structured actions do not also send the full Effective Capability Catalog prose block by default.
- [ ] Update Ollama-compatible provider serialization only when the selected Ollama model/runtime advertises native structured-action support; otherwise treat missing structured-action support as a readiness/compatibility issue or explicit text-action emulation capability, not an accidental prose fallback.
- [ ] Preserve a text-action emulation path only as an explicit ModelRuntime capability with its own validation and repair semantics; do not rely on accidental prose catalog fallback when native semantic actions are unavailable.
- [ ] Ensure trace level/content policy and unconditional secret redaction still apply to logical prompt renderings, provider text renderings, and structured action declarations: `minimal` omits full prompt bodies, `normal` may expose prompt metadata/section names without full bodies by default, and `verbose` may include the full logical render including Section 5 subject to content policy.
- [ ] Add tests proving native provider requests send structured tools/functions without duplicating the detailed catalog in prompt text, while verbose trace/report/debug logical rendering can still include Section 5 and lower trace levels avoid full prompt bodies by default.
